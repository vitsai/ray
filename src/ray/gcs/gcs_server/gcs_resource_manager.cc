...
void GcsResourceManager::HandleGetAllResourceUsage(
    ...
    rpc::SendReplyCallback send_reply_callback) {
  RAY_LOG(ERROR) << "The GetAllResourceUsage RPC has moved. "
                 << "Please use GetAllResourceUsageLegacy instead.";
  GCS_RPC_SEND_REPLY(send_reply_callback, reply, Status::Invalid());
}
